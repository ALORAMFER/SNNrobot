...
class RL_method():
        ...
        for epoch in range(self.EPOCH_NUM):
            ...
            for _ in range(self.STEP_MAX):
                ...
                if len(memory) == self.MEMORY_SIZE:  # メモリサイズ分溜まっていれば学習
                    if update_type == "CVI":
                        self.update_network_CVI(epoch, memory, Q_ast)
                    elif update_type == "DQN":
                        self.update_network_DQN(epoch, memory, Q_ast)
            ...
            total_reward_torch, total_reward_akida, total_reward_snn = self.utility.save_log(network_type,
                                                                                    action_type,
                                                                                    epoch, start,
                                                                                    total_rewards,
    ...
    def RL_main2(self, model_name, update_type, network_type, action_type):
        ...
        self.utility = Utility(self.beta, self.wbit, self.abit, self.LOG_FREQ, self.STEP_MAX, self.model, self.model_keras)
        self.utility2 = Utility(self.beta, self.wbit, self.abit, self.LOG_FREQ, self.STEP_MAX, self.model2, self.model_keras2)
        ...
        for epoch in range(self.EPOCH_NUM):#iteration: because it includes the updates
            ...
            for _ in range(self.STEP_MAX):
                ...
                if len(memory) == self.MEMORY_SIZE:  # メモリサイズ分溜まっていれば学習
                    if update_type == "CVI":
                        self.update_network_CVI(epoch, memory, Q_ast) #update qith one minibatch                        
                    elif update_type == "DQN":
                        self.update_network_DQN(epoch, memory, Q_ast) #update qith one minibatch
                    else:
                        ic("更新されていません")
                        exit()
                    # Q関数の更新
            ...
            for idx in range(self.STEP_MAX):#step_max*epoch: 50*100=5000
                ...
                if len(memory) == self.MEMORY_SIZE: # メモリサイズ分溜まっていれば学習
                    ...
                    self.update_network_Distill(epoch, memory, Q_ast2) #update with one minibatch
            ...
            total_rewards.append(total_reward)  # 累積報酬を記録
            total_reward_torch, total_reward_akida, total_reward_snn = self.utility2.save_log("DoReFa",
                                                                                    action_type,
                                                                                    epoch, start,
                                                                                    total_rewards,
                                                                                    total_step, model_PATH2,
                                                                                    total_reward_torch,
                                                                                    total_reward_akida,
                                                                                    total_reward_snn)
    ...
    def update_network_Distill(self, epoch, memory, Q_ast2):
        # RLによって学習モデルを更新する関数
    ...
    def update_network_CVI(self, epoch, memory, Q_ast):
        # RLによって学習モデルを更新する関数
